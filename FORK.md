...
| **Reconcile untracked windows continuously** | local | The launch-only sweeps healed login races but stopped after 60 s, so a window dropped mid-session stayed stranded until repaired by hand (seen 2026-08-11: three apps full-screen behind a half-tiled window after ~20 h uptime). Reconciliation now repeats every 5 s for the life of the process, each sweep followed by a reflow. A quiet sweep costs one window-list syscall and no accessibility calls; reflows are coalesced per screen, so a sweep with nothing to repair has no visible effect. |
...
| **Never cancel a half-finished layout pass** | local | Rapid events used to cancel in-flight frame assignments mid-pass, so one window could already sit at its new frame while its partner still held the previous layout — the classic “one full-screen, one half-screen” look even when both windows were tracked. Reflow requests are now coalesced: a pass runs to completion, then at most one more pass runs if anything asked again while it was busy. |
| **Catch up a stale Space instead of skipping the layout** | local | A reflow that found the screen’s Space out of date used to return without moving anything. It now updates the Space first, then lays out, so a missed space-change notification cannot leave windows stranded on the previous layout’s frames. |
| **Heal accidental small-window floats** | local | `float-small-windows` (threshold 850) is decided once at first sight and never re-checked. A main window that was briefly under 850×850 while its app launched stayed floating forever and was excluded from every layout — same symptom as an untracked window. Windows floated solely for size are tagged; each reconciliation sweep unfloats any of them that have since grown past the threshold. Blacklist floats and a manual toggle-float are never undone. | | 
